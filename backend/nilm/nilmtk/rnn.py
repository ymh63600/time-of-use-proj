        ...
        for appliance_name, power in train_appliances:
            ...
            if train_main.size > 0:
                ...
                if len(train_main) > 10:
                    ...
                    model_save_path = self.file_prefix + "-{}-final.h5".format("_".join(appliance_name.split()))
                    model.save("model/"+model_save_path)

    def disaggregate_chunk(self,test_main_list,model=None,do_preprocessing=True,appliance_params=None):

        if model is not None:
            self.models = model
        
        if appliance_params is not None:
